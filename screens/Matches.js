     ...
     <View style={styles.container}>
      <ScrollView>
        {this.state.chats.map((uri) => {
          return (
            // <TouchableOpacity key={uri} style={styles.imgRow} onPress={() => this.props.navigation.navigate("Chat", {user: uri.user})}>
            <TouchableOpacity style={styles.imgRow} onPress={() => this.props.navigation.navigate("Chat", {user: uri.user})}>
              ...
              <Text style={[styles.hold, styles.center]}>{uri.user.name}</Text>
            </TouchableOpacity>
          )
        })}
      </ScrollView>
